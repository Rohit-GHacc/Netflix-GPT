  ...
  return (
    <div>
      ...
      <form className = 'bg-black/75 absolute w-1/3 text-white top-1/6 left-1/3 py-10 rounded-lg'> 
        <h1 className = 'text-white ml-13 mb-5 p-2 text-3xl font-bold px-[1/4]'> {isRegistered ? "Sign In" : "Sign Up"} </h1>
        <div className = 'flex flex-col items-center'>
        { !isRegistered && <input type = 'text' placeholder= 'Full Name' className = 'p-4 m-4 bg-gray-900 opacity-50 rounded-sm w-3/4 mx-auto px-auto border border-gray-500'/>}
        ...
        <button className = 'bg-red-700 p-4 w-3/4 m-4 rounded-lg'> {isRegistered ? "Sign In" : "Sign Up"}</button>
        <h3 className = 'text-white '> Forgot Password? </h3>
        </div>
        <div className = 'text-white ml-15 my-5 cursor-pointer' onClick = {handleIsRegistered}> {isRegistered ? "New to Netflix? Sign up now":"Already registered? Sign in now"} </div>
      </form>
    </div>
  )
}
